...
clade_table <- read.table('./clade_profiles.txt', header = T, sep = '\t')
...
clade_table %>% filter(Species == 'Pocillopora acuta') %>% filter(C > 0 & D > 0) %>% nrow()
#########################################################
...
fig3a <- ggplot(data = otu_melt, aes(x = SampleID, y = value, fill = variable)) +
  ...
  scale_fill_manual(values = c(cool,warm,greys))

##################

##### fig 3b Symbiodiniaceae type profile composition in majority sequence #####
majority <- read.table('./majority sequences.txt', sep = '\t', header = TRUE, check.names = FALSE)
...
maj$variable <- factor(maj$variable, levels = c(colnames(majority)[7:84] %>% unique()))
...
fig3b <- ggplot(data = maj, aes(x = SampleID, y = value, fill = variable)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(),
        axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title.x = element_blank()) +
  scale_fill_manual(values = c(cool[3:25], warm[4:10], greys)) +
  labs(y= 'Relative abundance (%)')
  theme(legend.position = 'none') # remove legends

##### 1. sequence beta diversity #####
##### between-samples + figure 4a #####
##### cladocopium #####
c_unifrac_pcoa <- read.csv('./cladocopium_unifrac_sample_PCoA.csv')
type_profiles <- readxl::read_xlsx('./type_profiles_raw.xlsx')
...
fig4a_clad <- ggplot(data = c_unifrac_pcoa, aes(x = PC1, y = PC2, colour = Species, shape = Site)) +
  geom_point(size = 4) + 
  theme_bw() # fig 4a

# is the between-sample driven by host specificity?
c_dist_mat <- read.table('./unifrac_sample_square_C.txt')
row.names(c_dist_mat) <- c_dist_mat$V2

c_dist_mat$Species <- rep('coral', nrow(c_dist_mat))
c_dist_mat$Site <- rep('site', nrow(c_dist_mat))

for (row in 1:nrow(c_dist_mat)){
  uid = c_dist_mat[row,]$V2
  c_dist_mat[row, 'Species'] <- type_profiles[type_profiles$`ITS2 type profile` == uid, 'Species']
  c_dist_mat[row, 'Site'] <- type_profiles[type_profiles$`ITS2 type profile` == uid, 'Site']
}

c_dist_meta <- c_dist_mat %>% select(Species, Site)
c_dist_mat <- c_dist_mat %>% select(-V1, -V2, -Species, -Site)

# table S5.1
tableS5.1 <- vegan::adonis2(as.dist(c_dist_mat) ~ c_dist_meta$Species*c_dist_meta$Site)

df_c <- cmdscale(as.dist(c_dist_mat))
plot(df_c)

##### durusdinium #####
d_unifrac_pcoa <- read.csv('./durusdinium_unifrac_sample_PCoA.csv')
...
fig4a_durus <- ggplot(data = d_unifrac_pcoa, aes(x = PC1, y = PC2, colour = Species, shape = Site)) +
  geom_point(size = 4) + 
  theme_bw() 

# is the between-sample driven by host specificity?
d_dist_mat <- read.table('./unifrac_sample_square_D.txt')
row.names(d_dist_mat) <- d_dist_mat$V2

d_dist_mat$Species <- rep('coral', nrow(d_dist_mat))
d_dist_mat$Site <- rep('site', nrow(d_dist_mat))

for (row in 1:nrow(d_dist_mat)){
  uid = d_dist_mat[row,]$V2
  d_dist_mat[row, 'Species'] <- type_profiles[type_profiles$`ITS2 type profile` == uid, 'Species']
  d_dist_mat[row, 'Site'] <- type_profiles[type_profiles$`ITS2 type profile` == uid, 'Site']
}

d_dist_meta <- d_dist_mat %>% select(Species, Site)
d_dist_mat <- d_dist_mat %>% select(-V1, -V2, -Species, -Site)

# table S5.2
tableS5.2 <- vegan::adonis2(as.dist(d_dist_mat) ~ d_dist_meta$Species*d_dist_meta$Site)

df_d <- cmdscale(as.dist(d_dist_mat))
plot(df_d)

##### between-types + figure 4b ######
## cladocopium
cladocopium_types <- read.csv('./unifrac_profiles_PCoA_coords_C.csv')
...
ggplot(aes(x = PC1, y = PC2),  data = df_c_pca) + geom_point()
## durusdinium
durusdinium_types <- read.csv('./unifrac_profiles_PCoA_coords_D.csv')
...
# table S2
tableS2 <- vegan::adonis2(as.dist(d_dist_mat) ~ d_dist_meta$Species*d_dist_meta$Site)

df <- prcomp(d_dist_mat)
biplot(df)

##### 2. ecological beta diversity #####
types_table <- read.csv('./comb_abs_abund.csv', check.names = FALSE, header = T)
...
diplo_otu <- readxl::read_xlsx('./diplo_seq_absabund.xlsx') %>% as.data.frame()
...
diplo_envfit <- envfit(diplo_pca ~ DHW4 + BO_parmean + BO_ph + BO21_tempmean_ss + BO21_salinitymean_ss, data=diplo_meta,perm=999)
...
diplo_permanova_env <- adonis2(as.dist(diplo_ait) ~ DHW4 + BO_parmean + BO_ph + BO21_tempmean_ss + BO21_salinitymean_ss, data = diplo_meta)
diplo_permanova_env
...
plutea_otu <- readxl::read_xlsx('./plutea_seq_absabund.xlsx') %>% as.data.frame()
...
plutea_envfit <- envfit(plutea_pca ~ DHW4 + BO_parmean + BO_ph + BO21_tempmean_ss + BO21_salinitymean_ss, data=plutea_meta,perm=999)
...
plutea_permanova_env <- adonis2(as.dist(plutea_ait) ~ DHW4 + BO_parmean + BO_ph + BO21_tempmean_ss + BO21_salinitymean_ss, data = plutea_meta)
...
pacuta_otu <- readxl::read_xlsx('./pacuta_seq_absabund.xlsx') %>% as.data.frame()
...
pacuta_envfit <- envfit(pacuta_pca ~ DHW4 + BO_parmean + BO_ph + BO21_tempmean_ss + BO21_salinitymean_ss, data=pacuta_meta,perm=999)
...
pacuta_permanova_env <- adonis2(as.dist(pacuta_ait) ~ DHW4 + BO_parmean + BO_ph + BO21_tempmean_ss + BO21_salinitymean_ss, data = pacuta_meta)
...
pachy_otu <- readxl::read_xlsx('./pachy_seq_absabund.xlsx') %>% as.data.frame()
...
pachy_envfit <- envfit(pachy_pca ~ DHW4 + BO_parmean + BO_ph + BO21_tempmean_ss + BO21_salinitymean_ss, data=pachy_meta,perm=999)
...
pachy_permanova_env <- adonis2(as.dist(pachy_ait) ~ DHW4 + BO_parmean + BO_ph + BO21_tempmean_ss + BO21_salinitymean_ss, data = pachy_meta)
pachy_permanova_env
